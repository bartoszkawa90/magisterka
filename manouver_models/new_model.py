...
class BaseNet(nn.Module):
    def __init__(self, input_shape, actor_shape, device=torch.device("cuda")):
        ...
        # --- Learnable embedding for 3-way manouver ---
        self.manouver_emb = nn.Embedding(3, 32)

        # these get overridden in Actor/Critic
        self.head = nn.Identity()

    def extract_features(self, x, speed, manouver):
        # image normalization
        ...
        idx = manouver.to(self.device) if manouver is not None else torch.zeros(x.size(0), dtype=torch.long, device=self.device)
        manouver_features = self.manouver_emb(idx)

        return torch.cat([cnn_features, speed_features, manouver_features], dim=1)
...
class Actor(BaseNet):
    ...
    def forward(self, img, speed=None, manouver=None):
        feat = self.extract_features(img, speed, manouver)
